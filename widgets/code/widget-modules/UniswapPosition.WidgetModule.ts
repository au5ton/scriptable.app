...
function financialFormat(n: number): string {
  return n.toLocaleString(undefined,{'minimumFractionDigits':2,'maximumFractionDigits':2});
}

async function getUSDCPrice(): Promise<number> {
  const res = await GraphQL<{ bundle: { ethPriceUSD: string }}>('https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3',`{
    bundle(id: "1" ) {
      ethPriceUSD
    }
  }`);

  return Number(res.data.bundle.ethPriceUSD)
}
...
export interface CalculatedPositionData {
  ...
  currentPrice: number;
  currentPriceUnitDescription: string;
  inRange: boolean;
  createdTimestamp: number;
  gasCostToCreateUSD: number;
  profitUSD: number;
  averageEarningsPerDayUSD: number;
}

async function calculateUniswapPositionData(positionID: string): Promise<CalculatedPositionData> {
  ...
  const { unclaimedFeesToken0, unclaimedFeesToken1 } = (() => {
    ...
    const { 
      position: { 
        ...
        tickLower: { 
          feeGrowthOutside0X128: feeGrowthOutside0X128_lower,
          feeGrowthOutside1X128: feeGrowthOutside1X128_lower,
        },
        tickUpper: { 
          feeGrowthOutside0X128: feeGrowthOutside0X128_upper,
          feeGrowthOutside1X128: feeGrowthOutside1X128_upper,
        } 
      }, 
    } = data;
...
async function getUniswapPositionData(positionID: string): Promise<UniswapPositionData> {
