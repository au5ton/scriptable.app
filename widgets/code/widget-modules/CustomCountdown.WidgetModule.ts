...
const parseWidgetParameter = (param: string) => {
  ...
  const givenOptions: { [key: string]: string } = {};
  ...
  let date: string = _temp.toISOString();
  let title: string = 'Placeholder';
  let emoji: string = '🔔';
  let startColor: string = '#3050cc';
  let endColor: string = '#050530';

  if(givenOptions['date'] !== undefined) date = givenOptions['date'];
  if(givenOptions['title'] !== undefined) title = givenOptions['title'];
  if(givenOptions['emoji'] !== undefined) emoji = givenOptions['emoji'];
  if(givenOptions['startcolor'] !== undefined) startColor = givenOptions['startcolor'];
  if(givenOptions['endcolor'] !== undefined) endColor = givenOptions['endcolor'];
